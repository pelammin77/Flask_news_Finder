...
#        print self.html
    ...
    def grabArticle(self):
        ...
        for elem in self.html.findAll(True):
            ...
            unlikelyMatchString = " ".join(str(elem.get('id',''))+" ".join(str(elem.get('class','')) ))
...
#                pass
            ...
            if elem.name == 'div':
                s = elem.encode_contents(encoding='utf-8')

                decoded_S = s.decode("utf-8")
                if not self.regexps['divToPElements'].search(decoded_S):
                    elem.name = 'p'
...
#            print content
    ...
    def clean(self,e ,tag):
        ...
        for target in targetList:
            ...
            if isEmbed and self.regexps['videos'].search(target.renderContents()):
                continue
            target.extract()
    ...
    def get_article_content_html(self):
        return self.content

    def get_article_content_text(self):
...
url = "https://www.nytimes.com/2017/08/14/us/politics/trump-charlottesville-protest.html?partner=rss&emc=rss"
htmlcode = urllib.request.urlopen(url).read().decode('utf-8')
readability = Html_cleaner(htmlcode, url)
print(readability.get_article_title())
print(readability.get_article_content_html())

#print (readability.title)
#print (readability.content)
print(readability.get_article_content_text())
"""
